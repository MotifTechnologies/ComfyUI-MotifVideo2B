...
- `.plans/` — 작업 기억 (plan, context, checklist, log)
...
- 사용자 OK 후 즉시 03_checklist.md [x] 마킹 + 02_context.md 갱신 + 04_log.md 기록
...
#### 사용자 재지적 시 행동 규칙
사용자가 같은 문제를 다시 지적하면 (암묵적 REVISE):
1. 사과 대신 "원인: X, 수정: Y" 구조로 응답
2. 같은 지적 2회 이상: 표면적 수정 금지. 근본 원인 분석 후 보고
3. 요구사항 누락이 원인이면: 03_checklist.md에 검증 항목으로 추가
...
**recommender 자동 반영 (/auto 전용):** 전체 완료 후 recommender 호출 → 단순 제안(5줄 이내)은 반영 후 reviewer 호출 → PASS 시 커밋, 복잡한 제안은 이슈 등록. 일반 모드에서는 태스크 완료 워크플로우를 따른다.
...
1. 코드 변경 작업 완료 → **tester 호출** (테스트 코드 생성). tester에게는 요구사항(01_plan + 03_checklist 해당 항목) + 변경 파일 경로만 전달. 구현 내용(diff)은 전달 금지 (블라인드 테스트 원칙). 문서만 수정한 작업은 tester 스킵하고 2번으로 (위 분류표 참조).
...
6. Claude가 직접 수행 (OK 받은 즉시, 다음 작업 시작 전에 반드시 실행):
   ...
   - 04_log.md에 완료 항목명 + 변경 파일 + 핵심 결정/시행착오 append
7. (Stop hook이 [x] 증가 감지 → [HOOK:CHECKLIST_DONE] 발동 → 커밋)

> **Hook 강제 사항**: agent-tracker.sh가 planner/developer/tester/reviewer/recommender 호출을 추적합니다.
> - tester/reviewer 미호출 시 plan-gate가 다음 코드 수정 시 **차단** (exit 2)
...
- 인프라/운영: 확실하지 않으면 "확인 필요" 명시. "아마 괜찮을 것" 식의 낙관적 추측 금지.
  - 구체적 트리거: pod 재시작 필요 여부 판단, 노드 스케일 변경 권고, GPU/CPU 메모리 한계치 추정, 환경 재구성(환경변수·볼륨·네트워크) 영향 범위 판단, 프로세스 kill/restart 부작용 예측.
  - 판단 기준: 공식 문서·실측 데이터·knowledge 기록으로 뒷받침할 수 없으면 "확인 필요"로 표기.
...
### 독립 에이전트 설계 제약
- 독립 에이전트는 사용자 입력을 직접 대기하는 규칙을 가질 수 없다 (common.md Rule B 위반 + 교착 위험).
- 쓰기 작업이 필요하면 **제안 반환 → 메인이 사용자 승인 → 메인이 재호출** 패턴을 따른다.
- 독립 에이전트가 응답 없이 장시간 블로킹되면 교착을 의심한다. 메인은 에이전트 재호출 또는 사용자에게 타임아웃 보고.
...
## 작업 기억
- 큰 작업은 `.plans/YYYYMMDD-작업명/`에 계획서·맥락노트·체크리스트·작업로그 생성. `/task-plan` 참조.
- 세션 복구 시 `.plans/` 최신 폴더부터 읽기. `/restore` 참조.
- 플랜 문서 수정 시 01_plan → 02_context → 03_checklist 순서로 동기화.
- 04_log.md는 동기화 대상 아님 (append-only 시간순 기록).
...
- agent-tracker.sh (PostToolUse: Agent) — 에이전트 호출 추적 (planner/developer/tester/reviewer/recommender)
