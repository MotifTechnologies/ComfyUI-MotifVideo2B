"""P2.1: MotifVideoAttention state_dict key set == expected_attn_keys.json.
...
def test_single_block_state_dict_keys():
    num_heads, head_dim = _dims()
    attn = MotifVideoAttention(
        num_heads, head_dim, qk_norm="rms_norm", pre_only=True, added_kv=False
    )
    actual = set(attn.state_dict().keys())
    expected = set(EXPECTED["single_block"].keys())
    extra = actual - expected
    missing = expected - actual
    assert actual == expected, (
        f"single_block key mismatch. extra={sorted(extra)}, missing={sorted(missing)}"
    )


def test_dual_block_state_dict_keys():
    num_heads, head_dim = _dims()
    attn = MotifVideoAttention(
    ...
    missing = expected - actual
    assert actual == expected, (
        f"dual_block key mismatch. extra={sorted(extra)}, missing={sorted(missing)}"
    )
